...
import { Box, TextField, Button, InputAdornment, FormControl, Select, MenuItem, InputLabel } from '@mui/material';
...
const ExpenseForm = ({ addExpense, language = 'en' }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    addExpense({ description, amount: parseFloat(amount), paymentDate, effectiveDate, category, subcategory });
    setDescription('');
    setAmount('');
    setPaymentDate('');
    setEffectiveDate('');
  ...
  return (
    <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      <FormControl fullWidth required sx={{ mb: 0.1, backgroundColor: 'white' }}>
        <InputLabel id="category-label">{t(language, 'common.category')}</InputLabel>
        <Select
      ...
      <FormControl fullWidth required sx={{ mb: 0.1, backgroundColor: 'white' }}>
        <InputLabel id="subcategory-label">{t(language, 'common.subcategory')}</InputLabel>
        <Select
