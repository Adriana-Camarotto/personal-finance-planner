...
const Header = ({ language = 'en', onLanguageChange }) => {
    ...
    const languageFlag = language === 'pt-BR'
        ? { src: brazilFlag, alt: 'Brazil flag' }
        : { src: ukFlag, alt: 'United Kingdom flag' };
    const currentLanguageLabel = language === 'pt-BR' ? labels.portugueseBrazil : labels.english;
    ...
    return (
        <AppBar position="static">
            <Toolbar>
                ...
                <Box sx={{ padding: 2, backgroundColor: 'primary', color: 'white', display: 'flex', gap: 1 }}>
